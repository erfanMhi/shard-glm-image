...
OUT=${RING_IDS:-$HERE/../runs/ring_ids.txt}; mkdir -p "$(dirname "$OUT")"; [ -n "${APPEND:-}" ] || : > "$OUT"
# EXCLUDE_HOSTS=h1,h2 skips hosts that failed to boot earlier; hosts already in OUT are skipped too (APPEND mode)
...
EXCL="${EXCLUDE_HOSTS:-}"; [ -s "$OUT" ] && EXCL="$EXCL,$(awk '{print $5}' "$OUT" | paste -sd, -)"
EXCLUDE_HOSTS="$EXCL" python3 - "$N" "${PREF[@]}" > /tmp/ring_pick.txt <<'PY'
import json, sys, os
n = int(sys.argv[1]); pref = [s.replace("_", " ") for s in sys.argv[2:]]
excl = {int(h) for h in os.environ.get("EXCLUDE_HOSTS", "").split(",") if h.strip().isdigit()}
offers = [o for o in json.load(open("/tmp/ring_offers.json")) if o.get("host_id") not in excl]
best = {}
...
for o in picks:
    ...
    print(o["id"], st + (str(seen[st]) if seen[st] > 1 else ""), round(o["dph_total"], 3), o["host_id"])
PY
cnt=$(wc -l < /tmp/ring_pick.txt | tr -d ' '); [ "$cnt" -ge "$N" ] || { echo "only $cnt offers available (need $N):"; cat /tmp/ring_pick.txt; exit 1; }
tot=0
while read -r OFFER STATE PRICE HOST; do
  ID=$(bash "$HERE/rent.sh" "$OFFER" "ring-$(echo "$STATE" | tr 'A-Z' 'a-z')" 150 2>&1 | tail -1)
  [[ "$ID" =~ ^[0-9]+$ ]] && { echo "$ID $STATE $OFFER $PRICE $HOST" | tee -a "$OUT"; tot=$(python3 -c "print(round($tot+$PRICE,3))"); } || echo "RENT FAILED for $STATE offer $OFFER: $ID"
